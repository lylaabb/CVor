...
class PPO_RLOO:
    ...
    def update(self):
        ...
        for _ in range(self.K_epochs):
            ...
            # avg_except_current = logprobs.mean()

            # deps_w = (logprobs - logprobs.mean()) / (logprobs.std() + 1e-5)
            # deps_w = logprobs
            # 除以n-1来计算其他元素的平均值
            # n = len(deps_w)
            # deps_v = torch.zeros(n, device=device)
            # deps_v = (deps_w.sum() - deps_w) / (n-1)
            # deps_v = deps_w
            # deps = 1
            ...
            '''
            #########################################
            main part for CVor with RLOO form
            #########################################
            '''
            ...
            # n = len(loss)
            # deps_w = self.alpha * (loss.sum() - loss.detach()) / (n - 1)
        ...
        total_grad_all = total_grad_all / self.K_epochs
