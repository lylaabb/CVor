...
class PPO_RE:
    ...
    def update(self):
        ...
        for _ in range(self.K_epochs):
            ...
            # 计算所有logprobs的总和，并减去当前logprobs，得到除当前元素外的其他元素的和
            sum_except_current = logprobs.sum() + old_logprobs.sum() - logprobs - old_logprobs

            # 除以n-1来计算其他元素的平均值
            deps_v = - sum_except_current / (2 * (n - 1))

            deps = torch.exp(torch.exp(deps_w - deps_w.detach()) - torch.exp(deps_v - deps_v.detach()))

            # deps = torch.exp(deps_w - deps_w.detach()).mean()
            ...
            loss = - torch.min(surr1, surr2) * deps + deps ** 2 * 0.5 * self.MseLoss(state_values, rewards) \
                   - 0.01 * dist_entropy
            ...
            # 重参数化更新w
            # self.w = deps_w.detach()

            # Copy new weights into alpha policy
        self.policy_old.load_state_dict(self.policy.state_dict())
